      ...
      <div class="grid lg:grid-cols-2 gap-10 py-12 sm:py-16 lg:py-20 items-center">
        <div class="copy-section">
          ...
          <div class="mt-6 flex flex-wrap items-center justify-center text-center bg-red-700 gap-3 hero-actions"
            ...
            <a href="#contact"
              class="inline-flex items-center px-5 py-3 rounded-xl border border-slate-300 text-gray-900 font-semibold hover:bg-slate-50">{{ __('free_consultation') }}</a>
          </div>
          <div class="mt-8 flex items-center justify-center gap-6 text-sm text-gray-600 hero-metrics"
            style="justify-content:center;">
            <div class="flex items-center gap-2"><img src="./assets/up.svg" alt="up"
                width="25">{{ optional($post->meta->firstWhere('meta_key', 'مشروع محلي معتمد'))->getTranslation('meta_value', app()->getLocale()) }}
              مشروع <br /> محلي معتمد</div>
            <div class="flex items-center gap-2"><img src="./assets/up.svg" alt="up"
                width="25">{{ optional($post->meta->firstWhere('meta_key', ' نسبة فرص المتدربين'))->getTranslation('meta_value', app()->getLocale()) }}
              نسبة <br /> فرص المتدربين</div>
          </div>
        </div>
        <div class="relative">
