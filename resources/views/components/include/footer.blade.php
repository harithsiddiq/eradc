    ...
    <div class="grid md:grid-cols-4 gap-8">
      <div class="md:col-span-1">
        ...
        <div class="flex items-center gap-4">
          ...
          <a href="{{ $link['url'] ?? '#' }}" aria-label="{{ $platform }}"
            class="w-8 h-8 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-brand-600 transition-colors">
            <i class="{{
        ...
        $platform === 'linkedin' ? 'fi fi-brands-linkedin' : (
          $platform === 'instagram' ? 'fi fi-brands-instagram' : (
            $platform === 'tiktok' ? 'fi fi-brands-tik-tok' : (
              $platform === 'facebook' ? 'fi fi-brands-facebook' : 'fi fi-brands-link'))))
                }}"></i>
          </a>
          @endforeach
          @endif
      ...
      <div>
        ...
        <div class="space-y-3 text-sm">
          <div class="flex items-start gap-3"><svg class="w-4 h-4 text-brand-500 mt-0.5 flex-shrink-0"
              fill="currentColor" viewBox="0 0 20 20">
              <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z" />
              <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" />
            </svg>
            <div>
              <p class="text-gray-400">{{ $footer->support_email }}</p>
              <p class="text-gray-400">{{ $footer->admissions_email }}</p>
            </div>
          </div>
          <div class="flex items-start gap-3"><svg class="w-4 h-4 text-brand-500 mt-0.5 flex-shrink-0"
              fill="currentColor" viewBox="0 0 20 20">
            ...
            <div>
              <p class="text-gray-400">{{ $footer->phone ?? '+966 50 123 4567' }}</p>
              <p class="text-gray-400 text-xs">{{ $footer->hours ?? 'الأحد-الخميس: 9ص-6م' }}</p>
            </div>
          </div>
          <div class="flex items-start gap-3"><svg class="w-4 h-4 text-brand-500 mt-0.5 flex-shrink-0"
              fill="currentColor" viewBox="0 0 20 20">
            ...
            <div>
              <p class="text-gray-400">
                {{ is_array($footer->address_line1) ? ($footer->address_line1[app()->getLocale()] ?? reset($footer->address_line1)) : $footer->address_line1 }}
              </p>
              <p class="text-gray-400">
                {{ is_array($footer->address_line2) ? ($footer->address_line2[app()->getLocale()] ?? reset($footer->address_line2)) : $footer->address_line2 }}
              </p>
            </div>
          </div>
        </div>
    ...
    <div class="border-t border-gray-800 mt-8 pt-8 flex flex-col sm:flex-row justify-between items-center">
      ...
      <div class="flex items-center gap-6 mt-4 sm:mt-0 text-sm">
        @foreach(($footer->policies ?? []) as $p)
          <a href="{{ $p['url'] ?? '#' }}"
            class="text-gray-400 hover:text-white transition-colors">{{ $p['label'] ?? '' }}</a>
        @endforeach
      </div>
    </div>
  </div>
